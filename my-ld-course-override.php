...
if ( is_singular( 'sfwd-quiz' ) ) {
    ...
    wp_enqueue_script(
        ...
        [ 'jquery' ],
        '1.1', // sube versión para evitar caché
        true
    );
    ...
    $type = ( $quiz_id === $first_quiz_id ) ? 'first' : 'final';
...
function handle_enviar_correo_final_quiz() {
	...
	$arrow     = $variation >= 0 ? '▲' : '▼';
	$days      = 0;
	if ( ! empty( $first_perf['date'] ) && strtotime( $first_perf['date'] ) && $completed_ts ) {
		$days = max( 1, floor( ( $completed_ts - strtotime( $first_perf['date'] ) ) / DAY_IN_SECONDS ) );
	}
